...
class UserConversationsScreen extends Component {
  constructor(props) {
    super(props);
    this.state = {
      loading: false,
    ...
    return (
      <View style={{ flex: 1 }}>
        {this.state.data &&
          ...
          renderItem={({ item }) => (
            <>
            <TouchableOpacity 	onPress={() => this.props.navigation.navigate('Chat Details',{userid:global.user._id, workerid:item.worker_id._id})} >
             <View style={styles.eachMsg}>
             <Image source={require(`./../../assets/images/avatar.png`)} style={styles.userPic} />
             <View style={styles.v}>
               <Text style={styles.msgTxt}>ID: {item.worker_id._id}</Text>
               <Text style={styles.msgTxt}>Name: {item.worker_id.name}</Text>
              </View>
           </View>     
          </TouchableOpacity>
          </>
          )}
          keyExtractor={item => item.worker_id._id}
